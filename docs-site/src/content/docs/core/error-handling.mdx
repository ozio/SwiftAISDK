...
description: Handle validation, provider, retry, timeout, abort, and output errors.
---

import { Code } from '@astrojs/starlight/components';
import errorHandlingExample from '../../../../../Examples/Sources/ErrorHandling/main.swift?raw';

Every facade call is `async throws`, and streams are `AsyncThrowingStream`s. Catch specific error types when you need structured fields, then fall back to `Error` for logging.

<Code code={errorHandlingExample} lang="swift" title="Examples/Sources/ErrorHandling/main.swift" />
...
For `AI.streamText`, pass `AIStreamTimeoutConfiguration` to add a per-model-step
...
Streaming retries only happen before the first emitted part. Once the stream has yielded a part, later provider errors are surfaced through the stream instead of replaying already-delivered output.

Language streams distinguish provider-reported error events from failures that stop the transport or parser:

- `.error(message:rawValue:)` is a repeatable in-band `LanguageStreamPart`. The full stream forwards every such part and records `error` as the provisional step outcome; a later explicit provider terminal reason remains authoritative.
...
An in-band error counts as an emitted part, so a model call is never retried after the caller could have observed one. Consumers that need provider error payloads should use the full `LanguageStreamPart` sequence rather than `toTextStream()`.
...
Provider-controlled download URLs are followed manually. SwiftAISDK validates
every redirect target before issuing the next request, preserves provider
credentials for same-origin hops, and removes them when the origin changes.
This does not yet claim resolver-backed DNS answer pinning; that remains a
transport-level gap.

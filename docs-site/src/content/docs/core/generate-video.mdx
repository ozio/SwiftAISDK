...
A caller-supplied `Idempotency-Key` header wins. Black Forest Labs FLUX 3 and
Fal are operation adapters; callers can also invoke `startVideoGeneration` and
`videoGenerationStatus` directly and persist the returned JSON operation.
...
parameter. The webhook factory is not invoked for providers without native
webhook support; Black Forest Labs emits an unsupported warning and falls back
to polling. `timeoutMilliseconds` also bounds the webhook wait, and caller
cancellation closes either path.

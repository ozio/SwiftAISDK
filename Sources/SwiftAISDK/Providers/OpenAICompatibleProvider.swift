...
public final class OpenAICompatibleProvider: AIProvider, @unchecked Sendable {
    ...
    static func buildHeaders(providerID: String, authorization: AuthorizationStyle, settings: ProviderSettings, userAgentSuffix: String? = nil) throws -> [String: String] {
        ...
        if providerID == "anthropic" {
            return withUserAgentSuffix(headers, "ai-sdk/anthropic/4.0.16")
        }
        if providerID == "google.generative-ai" {
            return withUserAgentSuffix(headers, "ai-sdk/google/4.0.18")
        }
        if providerID == "moonshotai" {
            return withUserAgentSuffix(headers, "ai-sdk/moonshotai/3.0.15")
        }
        if providerID == "cerebras" {
            return withUserAgentSuffix(headers, "ai-sdk/cerebras/3.0.12")
        }
        if providerID == "deepseek" {
            return withUserAgentSuffix(headers, "ai-sdk/deepseek/3.0.12")
        }
        if providerID == "baseten" {
            return withUserAgentSuffix(headers, "ai-sdk/baseten/2.0.12")
        }
        if providerID == "groq" {
            return withUserAgentSuffix(headers, "ai-sdk/groq/4.0.12")
        }
        if providerID == "mistral" {
            return withUserAgentSuffix(headers, "ai-sdk/mistral/4.0.13")
        }
        if providerID == "cohere" {
            return withUserAgentSuffix(headers, "ai-sdk/cohere/4.0.11")
        }
        if providerID == "elevenlabs" {
            return withUserAgentSuffix(headers, "ai-sdk/elevenlabs/3.0.12")
        }
        if providerID == "assemblyai" {
            return withUserAgentSuffix(headers, "ai-sdk/assemblyai/3.0.11")
        }
        if providerID == "deepgram" {
            return withUserAgentSuffix(headers, "ai-sdk/deepgram/3.0.11")
        }
        if providerID == "lmnt" {
            return withUserAgentSuffix(headers, "ai-sdk/lmnt/3.0.11")
        }
        if providerID == "hume" {
            return withUserAgentSuffix(headers, "ai-sdk/hume/3.0.11")
        }
        if providerID == "revai" {
            return withUserAgentSuffix(headers, "ai-sdk/revai/3.0.11")
        }
        if providerID == "gladia" {
            return withUserAgentSuffix(headers, "ai-sdk/gladia/3.0.11")
        }
        if providerID == "fal" {
            return withUserAgentSuffix(headers, "ai-sdk/fal/3.0.12")
        }
        if providerID == "bytedance" {
            return withUserAgentSuffix(headers, "ai-sdk/bytedance/2.0.13")
        }
        if providerID == "voyage" {
            return withUserAgentSuffix(headers, "ai-sdk/voyage/2.0.11")
        }
        if providerID == "alibaba" {
            return withUserAgentSuffix(headers, "ai-sdk/alibaba/2.0.14")
        }
        if providerID == "luma" {
            return withUserAgentSuffix(headers, "ai-sdk/luma/3.0.12")
        }
        if providerID == "klingai" {
            return withUserAgentSuffix(headers, "ai-sdk/klingai/4.0.12")
        }
        if providerID == "replicate" {
            return withUserAgentSuffix(headers, "ai-sdk/replicate/3.0.12")
        }
        if providerID == "black-forest-labs" {
            return withUserAgentSuffix(headers, "ai-sdk/black-forest-labs/2.0.11")
        }
        if providerID == "prodia" {
            return withUserAgentSuffix(headers, "ai-sdk/prodia/2.0.12")
        }
        if providerID == "quiverai" {
            return withUserAgentSuffix(headers, "ai-sdk/quiverai/2.0.11")
        }
        if providerID == "togetherai" {
            return withUserAgentSuffix(headers, "ai-sdk/togetherai/3.0.13")
        }
        if providerID == "fireworks" {
            return withUserAgentSuffix(headers, "ai-sdk/fireworks/3.0.13")
        }
        if providerID == "deepinfra" {
            return withUserAgentSuffix(headers, "ai-sdk/deepinfra/3.0.12")
        }
        if providerID == "xai" {
            return withUserAgentSuffix(headers, "ai-sdk/xai/4.0.16")
        }

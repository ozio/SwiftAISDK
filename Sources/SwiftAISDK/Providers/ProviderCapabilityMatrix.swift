...
public enum AIProviderCapabilities {
    public static let markdownSnapshotDate = "2026-08-17"
    ...
    public static let all: [AIProviderCapabilityRow] = [
        ...
        providerRow("deepinfra", "@ai-sdk/deepinfra", ["AIProviders.deepInfra"], [.language, .completion, .embedding, .image], notes: "DeepInfra mirrors upstream 3.0.30: `DEEPINFRA_API_KEY`, bearer auth, custom base URL and headers, versioned user-agent suffix, callable/chat/language aliases, completion/embedding/textEmbedding aliases, image/imageModel aliases, `deepinfra.chat`/`.completion`/`.embedding`/`.image` provider IDs, `/openai/*` language/embedding routes, `/inference/{model}` image generation, OpenAI-compatible multipart image edits, Gemma/Gemini reasoning usage correction, `providerOptions.deepinfra` passthrough, max-one generated image per call, and response metadata."),
        ...
        providerRow("openai", "@ai-sdk/openai", ["AIProviders.openAI"], [.language, .completion, .embedding, .image, .transcription, .speech], files: true, skills: true, notes: "OpenAI mirrors upstream 4.0.42 across Chat Completions, Responses language models, completions, embeddings, images, speech, transcription, files, skills, hosted Responses tools, provider tool helpers, organization/project headers, structured output, usage details, response/provider metadata, warnings, raw stream chunks, provider references, Responses allowedTools/tool-search/namespace handling including blocked-domain filters, server-side compaction items, retryable pre-output stream failures, and Chat file-part mapping for image/audio/PDF inputs. File uploads serialize expiry options as nested `expires_after[anchor]` and `expires_after[seconds]` multipart fields; Chat metadata ignores zero-valued Azure content-filter placeholder timestamps while generic compatible providers preserve explicit epoch timestamps. The latest pass JSON-encodes function outputs that declare output schemas, keeps streaming output-item identity stable across rotating IDs through `output_index`, supports `serviceTier: fast`, and adds Responses programmatic tool calling with caller controls, output schemas, streamed/generated program items, and multi-step continuation; preserves stored tool-search IDs; and applies reasoning, sampling, service-tier, and image defaults to recognizable future OpenAI model versions. The current pass also adds terminal compaction triggers, previous-response function/output pairing, storage-disabled shell replay, and MCP approval reference deduplication. Chat reasoning models strip unsupported sampling and penalty fields with upstream warnings, while `topLogprobs` is always omitted even when GPT-5.1+ permits non-reasoning parameters at effort `none`. Realtime/WebSocket models, ProviderV4 type names, ESM-only packaging, Node 22 engines, and workflow serialization helpers remain JS-runtime/package-surface concerns."),
        providerRow("openai-compatible", "@ai-sdk/openai-compatible", ["AIProviders.openAICompatible"], [.language, .completion, .embedding, .image], notes: "Generic OpenAI-compatible factory mirrors upstream 3.0.30: caller supplies provider ID and base URL, custom headers/query params, versioned user-agent suffix, upstream surface IDs, chat/completion/embedding/image models, providerOptions namespace unwrapping with raw and camelCase keys, deprecated raw-key warnings, structured outputs, stream usage, response/provider metadata, camelCase provider metadata namespace selection when camelCase providerOptions are used, extensible image provider options, and provider-controlled image response formats instead of a forced `response_format`, non-negative text-token usage when reasoning counts exceed completion counts, and ID/index/latest streamed tool-call correlation. ProviderV4 type names, ESM-only packaging, Node 22 engines, optional JS headers config, and workflow serialization helpers are JS-only upstream concerns."),

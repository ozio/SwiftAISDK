...
| KlingAI | `@ai-sdk/klingai/src/klingai-provider.ts`, `klingai-auth.ts`, `klingai-video-model.ts`, `klingai-video-model-options.ts` | `AIProviders.klingAI`, `AIProviders.klingai`, `KlingAIVideoModel`. Video generation maps upstream text-to-video, image-to-video, and motion-control suffix routing; `providerOptions.klingai` controls mode, polling, negative prompts, camera, multi-shot, voice, element, mask, watermark, and passthrough fields; standard image/duration/aspect ratio behavior follows mode-specific upstream warnings; results preserve task/video provider metadata, response metadata, `KLINGAI_ACCESS_KEY`/`KLINGAI_SECRET_KEY` JWT auth, and abort signals for create/poll requests. | `KlingAIProviderTests.swift`, `ProviderAbortPropagationTests.swift` |

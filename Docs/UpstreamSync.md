...
| Telemetry | Map upstream `telemetry`/`experimental_telemetry` to `AITelemetryOptions`, `AITelemetryIntegration`, and `AITelemetry.register(...)`. Per-call integrations override global integrations; `isEnabled: false` disables event dispatch. Current Swift events cover non-streaming facade start, retry, end, and error lifecycle events with input/output recording flags, function ID, metadata, usage, warnings, provider metadata, and response metadata. Keep stream telemetry, step/tool execution callbacks, object-specific event names, and execute wrappers as follow-up passes. |

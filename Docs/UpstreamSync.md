...
| Native response metadata | Use `aiResponseMetadata(...)` when native providers can expose upstream response metadata. It derives IDs/model IDs from provider JSON, preserves response headers and raw JSON bodies, and uses the current call time when the provider body has no `created` timestamp. Anthropic language, Google Generative AI language/embedding/image/video/files, and Google Vertex language/embedding/image/video preserve metadata on results; language streams emit response metadata before model deltas. |

...
| AI facade | `AI.generateText`, `AI.streamText`, `AI.generateObject`, `AI.embed`, `AI.embedMany`, `AI.generateImage`, `AI.transcribe`, `AI.generateSpeech`, `AI.generateVideo`, `AI.rerank`, `AI.uploadFile`, and `AI.uploadSkill` are product-level wrappers over the model/client protocols. Non-streaming facade calls use `AIRetryPolicy` with upstream-style default `maxRetries: 2` for retryable transient errors. `AI.generateText` can also execute typed `AITool` callbacks across multiple steps and append assistant tool-call/tool-result messages. `AI.generateObject` sets JSON response-format hints and decodes `Decodable` results. Stream-side tool execution, stream-object, retry-after headers, and telemetry behavior are still follow-up product passes. |
...
| Object generation | Keep `AI.generateObject` aligned with upstream `generateObject` / `Output.object`: request native JSON response formats when the provider supports them, parse/repair raw text into JSON, then validate through Swift decoding. Future passes should add schema adapter protocols, array/enum/no-schema strategies, and streaming partial object support. |

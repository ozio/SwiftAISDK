...
| Native response metadata | Use `aiResponseMetadata(...)` when native providers can expose upstream response metadata. Anthropic language, Google Generative AI language/embedding, and Google Vertex language/embedding preserve response headers and raw JSON bodies on generate/embed results and emit stream response metadata before model deltas. |

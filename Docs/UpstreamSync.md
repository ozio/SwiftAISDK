...
| Deepgram, ElevenLabs, Hume, LMNT, RevAI, Gladia, AssemblyAI | Audio provider packages | Provider-specific transcription/speech models. Deepgram maps `providerOptions.deepgram` for transcription and speech, keeps raw-audio listen requests, validates TTS encoding/container/sample-rate/bit-rate combinations with upstream-style warnings, warns that TTS voices are selected through the model ID, and forwards abort signals for both surfaces. | `AudioProviderTests.swift`, `DeepgramProviderTests.swift`, `ProviderAbortPropagationTests.swift` |

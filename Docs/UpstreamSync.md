...
| Object generation | Keep `AI.generateObject` and `AI.streamObject` aligned with upstream object output: request native JSON response formats when the provider supports them, parse/repair raw text into JSON, then validate through Swift decoding and `AIJSONSchemaValidator`. Current streaming support emits text deltas, repaired partial `JSONValue` objects, and a final decoded object. Future passes should add schema adapter protocols, array/enum/no-schema strategies, and typed partial object support. |

...
| Tools | Keep provider-defined tool builders beside the provider model. Test both tool schema and required beta/header behavior. Upstream `experimental_refineToolInput` maps to `AITool.refineArguments` before a Swift tool's execute callback. |

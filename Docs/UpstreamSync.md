...
| Image warnings | OpenAI-compatible image models return unsupported warnings for top-level `aspectRatio` and `seed`; these settings are intentionally not forwarded to OpenAI-style image endpoints. Hyphenated raw provider option keys return a deprecated warning that points to the camelCase key. |

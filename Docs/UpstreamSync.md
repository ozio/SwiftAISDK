...
| MCP client | `MCPClient` mirrors `@ai-sdk/mcp` for initialize, tool listing, tool calls, cached tool definitions, conversion to dynamic `AITool`, MCP content-to-model-output conversion for text/image/unknown content parts, resource listing/reading, resource templates, prompt listing/getting, and incoming `elicitation/create` requests. `MCPHTTPTransport` covers Streamable HTTP request/response semantics: protocol-version headers, JSON and SSE responses, `mcp-session-id` persistence, DELETE session termination, and inbound SSE requests answered through `setRequestHandler`. Use `AIStreamingTransport` for true long-lived SSE; `URLSessionTransport` implements it, while plain `AITransport` keeps buffered fallback parsing. MCP SSE reconnection/resumption, OAuth, and deeper provider-native multimodal model-output serialization remain follow-up MCP passes. |

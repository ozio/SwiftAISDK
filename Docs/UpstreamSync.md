...
| Telemetry | Map upstream `telemetry`/`experimental_telemetry` to `AITelemetryOptions`, `AITelemetryIntegration`, and `AITelemetry.register(...)`. Per-call integrations override global integrations; `isEnabled: false` disables event dispatch. Current Swift events cover non-streaming facade start, retry, end, and error lifecycle events plus `streamText`/`streamObject` start, end, and error events, with input/output recording flags, function ID, metadata, usage, warnings, provider metadata, and response metadata. Keep step/tool execution callbacks, object-specific event names, and execute wrappers as follow-up passes. |
| Object generation | Keep `AI.generateObject` and `AI.streamObject` aligned with upstream object output: request native JSON response formats when the provider supports them, parse/repair raw text into JSON, then validate through Swift decoding and `AIJSONSchemaValidator`. Object failures should stay typed through `AIObjectGenerationError` with strategy, kind, path, text, and repair-attempt metadata. Array output wraps the element schema as an object with an `elements` array, enum output wraps allowed values as an object with a `result` string, and no-schema output requests JSON without a schema and returns raw `JSONValue`. Current streaming support emits text deltas, repaired partial `JSONValue` objects, and a final decoded object. Future passes should add schema adapter protocols, streaming array/enum/no-schema variants, and typed partial object support. |

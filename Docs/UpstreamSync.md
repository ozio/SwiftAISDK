...
| AI facade | `AI.generateText`, `AI.streamText`, `AI.embed`, `AI.embedMany`, `AI.generateImage`, `AI.transcribe`, `AI.generateSpeech`, `AI.generateVideo`, `AI.rerank`, `AI.uploadFile`, and `AI.uploadSkill` are product-level wrappers over the model/client protocols. `AI.generateText` can also execute typed `AITool` callbacks across multiple steps and append assistant tool-call/tool-result messages. Retry, stream-side tool execution, object-generation, and telemetry behavior are still follow-up product passes. |

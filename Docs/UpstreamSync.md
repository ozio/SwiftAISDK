...
| AI facade | `AI.generateText`, `AI.streamText`, `AI.generateObject`, `AI.streamObject`, `AI.embed`, `AI.embedMany`, `AI.generateImage`, `AI.transcribe`, `AI.generateSpeech`, `AI.generateVideo`, `AI.rerank`, `AI.uploadFile`, and `AI.uploadSkill` are product-level wrappers over the model/client protocols. Non-streaming facade calls use `AIRetryPolicy` with upstream-style default `maxRetries: 2` for retryable transient errors. `AI.generateText` and `AI.streamText` can execute typed `AITool` callbacks across multiple steps, mark dynamic tool calls/results, refine parsed tool arguments before execution, run Swift tool approval policy, and append assistant tool-call/tool-result messages. OpenAI Responses provider-executed MCP approvals round-trip through `AIToolApprovalRequest`/`AIToolApprovalResponse`. `AI.generateObject` sets JSON response-format hints and decodes `Decodable` results. `AI.streamObject` streams JSON text deltas, best-effort partial `JSONValue` objects, and a final decoded object. Typed partial objects, retry-after headers, and telemetry behavior are still follow-up product passes. |
...
| Object generation | Keep `AI.generateObject` and `AI.streamObject` aligned with upstream object output: request native JSON response formats when the provider supports them, parse/repair raw text into JSON, then validate through Swift decoding. Current streaming support emits text deltas, repaired partial `JSONValue` objects, and a final decoded object. Future passes should add schema adapter protocols, array/enum/no-schema strategies, and typed partial object support. |

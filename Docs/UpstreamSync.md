...
| AI facade | `AI.generateText`, `AI.streamText`, `AI.generateObject`, `AI.embed`, `AI.embedMany`, `AI.generateImage`, `AI.transcribe`, `AI.generateSpeech`, `AI.generateVideo`, `AI.rerank`, `AI.uploadFile`, and `AI.uploadSkill` are product-level wrappers over the model/client protocols. `AI.generateText` can also execute typed `AITool` callbacks across multiple steps and append assistant tool-call/tool-result messages. `AI.generateObject` sets JSON response-format hints and decodes `Decodable` results. Retry, stream-side tool execution, stream-object, and telemetry behavior are still follow-up product passes. |

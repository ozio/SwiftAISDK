...
| AssemblyAI | `@ai-sdk/assemblyai` audio provider package | `AIProviders.assemblyAI`, `AssemblyAITranscriptionModel`. Maps `providerOptions.assemblyai`, submits upload URLs to `/v2/transcript`, maps upstream transcript option keys, keeps final GET response metadata and transcription details, and forwards abort signals across upload, submit, and poll requests. | `AssemblyAIProviderTests.swift`, `ProviderAbortPropagationTests.swift` |

...
| Mistral | `@ai-sdk/mistral` | Provider-specific Swift models plus registry helpers. Mistral maps v4 sampling warnings, seed, JSON response format, reasoning effort, function tools/tool choice, response metadata, stream lifecycle parts, abort signals, assistant `tool_calls`, and role `tool` messages with upstream-style `modelOutput ?? result` content. | `CohereMistralVoyageTests.swift`, `ProviderAbortPropagationTests.swift` |

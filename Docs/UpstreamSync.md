...
| MCP client | `MCPClient` mirrors `@ai-sdk/mcp` for initialize, tool listing, tool calls, cached tool definitions, and conversion to dynamic `AITool`. `MCPHTTPTransport` covers simple JSON-RPC-over-HTTP servers; custom transports can implement `MCPTransport`. Resources, prompts, elicitation, SSE/session handling, and richer content-to-model-output conversion remain follow-up MCP passes. |

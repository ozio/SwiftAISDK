...
| AI facade | `AI.generateText`, `AI.streamText`, `AI.generateObject`, `AI.streamObject`, `AI.embed`, `AI.embedMany`, `AI.generateImage`, `AI.transcribe`, `AI.generateSpeech`, `AI.generateVideo`, `AI.rerank`, `AI.uploadFile`, and `AI.uploadSkill` are product-level wrappers over the model/client protocols. Non-streaming facade calls use `AIRetryPolicy` with upstream-style default `maxRetries: 2` for retryable transient errors. `AI.generateText` can also execute typed `AITool` callbacks across multiple steps and append assistant tool-call/tool-result messages. `AI.generateObject` sets JSON response-format hints and decodes `Decodable` results. `AI.streamObject` streams JSON text deltas, best-effort partial `JSONValue` objects, and a final decoded object. Stream-side tool execution, typed partial objects, retry-after headers, and telemetry behavior are still follow-up product passes. |

...
| Telemetry | Map upstream `telemetry`/`experimental_telemetry` to `AITelemetryOptions`, `AITelemetryIntegration`, and `AITelemetry.register(...)`. Per-call integrations override global integrations; `isEnabled: false` disables event dispatch and execute wrapping. Current Swift events cover non-streaming facade and stream facade start, retry, end, and error lifecycle events, including `ai.generateObject` object output events, plus `stepStart`, `stepEnd`, `toolStart`, `toolEnd`, and `toolError` for `generateText`/`streamText` tool loops, with input/output recording flags, function ID, metadata, usage, warnings, provider metadata, and response metadata. `AITelemetryIntegration.executeLanguageModelCall` wraps language model generate/stream calls; `executeTool` wraps Swift tool execution, composing integrations in upstream order with the last integration outermost. |

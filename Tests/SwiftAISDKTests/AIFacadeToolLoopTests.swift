...
@Test func aiGenerateTextReturnsToolArgumentRefinementFailuresAsToolResults() async throws {
    ...
    #expect(toolResult.result["value"]?.stringValue?.contains("could not repair input") == true)
}
...
@Test func aiGenerateTextAddsDeniedApprovalResultAndContinues() async throws {
    ...
    #expect(model.requests.count == 2)
    #expect(model.requests[1].messages[1].content == [.toolCall(toolCall)])
    #expect(model.requests[1].messages[2].content == [.toolResult(deniedResult)])
}

...
@Test func aiGenerateImageForwardsRequestAndReturnsMetadataLikeUpstream() async throws {
    ...
    #expect(result.calls[0].urls == ["https://example.com/image.png"])
    #expect(result.calls[0].base64Images == [Data([1, 2, 3]).base64EncodedString()])
    #expect(result.calls[0].warnings == [warning])
    #expect(result.calls[0].usage == TokenUsage(inputTokens: 10, outputTokens: 0, totalTokens: 10))
    #expect(result.calls[0].providerMetadata["testProvider"]?["images"]?[0]?["revisedPrompt"]?.stringValue == "test-revised-prompt")
    #expect(result.calls[0].responseMetadata == responseMetadata)

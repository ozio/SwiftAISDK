...
@Test func lumaImageSubmitsPollsAndDownloadsImage() async throws {
    ...
    #expect(requests[0].headers["user-agent"] == "ai-sdk/luma/3.0.35")
    ...
    #expect(requests[1].headers["user-agent"] == "ai-sdk/luma/3.0.35")
...
@Test func lumaAppendsVersionedUserAgentToCustomHeader() async throws {
    ...
    let requests = await transport.requests()
    #expect(requests[0].headers["authorization"] == "Bearer luma-key")
    #expect(requests[0].headers["user-agent"] == "CustomApp/1.0 ai-sdk/luma/3.0.35")
    #expect(requests[1].headers["authorization"] == "Bearer luma-key")
    #expect(requests[1].headers["user-agent"] == "CustomApp/1.0 ai-sdk/luma/3.0.35")
    #expect(requests[2].headers["authorization"] == nil)
    #expect(requests[2].headers["user-agent"] == nil)
}

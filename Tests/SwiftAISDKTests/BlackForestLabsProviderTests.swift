...
@Test func blackForestLabsAppendsVersionedUserAgentToCustomHeader() async throws {
    ...
    let requests = await transport.requests()
    #expect(requests[0].headers["x-key"] == "bfl-key")
    #expect(requests[0].headers["user-agent"] == "CustomApp/1.0 ai-sdk/black-forest-labs/2.0.26")
    #expect(requests[1].headers["x-key"] == "bfl-key")
    #expect(requests[1].headers["user-agent"] == "CustomApp/1.0 ai-sdk/black-forest-labs/2.0.26")
    #expect(requests[2].headers["x-key"] == nil)
    #expect(requests[2].headers["user-agent"] == nil)
}
...
@Test func blackForestLabsSendsCredentialsToTrustedBFLClusterURLs() async throws {
    ...
    #expect(requests[2].headers["user-agent"] == "ai-sdk/black-forest-labs/2.0.26")
}

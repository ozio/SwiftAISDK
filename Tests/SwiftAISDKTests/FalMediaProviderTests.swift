...
@Test func falImageUsesRunEndpoint() async throws {
    ...
    #expect(request.headers["user-agent"] == "ai-sdk/fal/3.0.35")
...
@Test func falAppendsVersionedUserAgentToCustomHeader() async throws {
    ...
    #expect(requests[0].headers["user-agent"] == "CustomApp/1.0 ai-sdk/fal/3.0.35")
    #expect(requests[1].headers["authorization"] == nil)
    #expect(requests[1].headers["user-agent"] == nil)
}
...
@Test func falVideoUsesQueueAndResponseURL() async throws {
    ...
    #expect(requests[1].headers["user-agent"] == "ai-sdk/fal/3.0.35")
    ...
    #expect(requests[2].headers["user-agent"] == "ai-sdk/fal/3.0.35")
}
...
@Test func falVideoDoesNotSendCredentialsToForeignResponseURL() async throws {
    ...
    #expect(requests[1].headers["authorization"] == nil)
    #expect(requests[1].headers["user-agent"] == "CustomApp/1.0 ai-sdk/fal/3.0.35")
}
...
@Test func falSpeechAndTranscriptionUseNativeFalEndpoints() async throws {
    ...
    #expect(speechRequests[0].headers["user-agent"] == "ai-sdk/fal/3.0.35")
    ...
    #expect(transcriptionRequests[0].headers["user-agent"] == "ai-sdk/fal/3.0.35")
    ...
    #expect(transcriptionRequests[1].headers["user-agent"] == "ai-sdk/fal/3.0.35")
}

...
@Test func klingAIT2VSubmitsPollsAndPreservesMetadata() async throws {
    ...
    #expect(requests[0].headers["user-agent"] == "ai-sdk/klingai/4.0.42")
    ...
    #expect(requests[1].headers["user-agent"] == "ai-sdk/klingai/4.0.42")
}
...
@Test func klingAIAppendsVersionedUserAgentToCustomHeader() async throws {
    ...
    let requests = await transport.requests()
    #expect(requests[0].headers["authorization"] == "Bearer kling-token")
    #expect(requests[0].headers["user-agent"] == "CustomApp/1.0 ai-sdk/klingai/4.0.42")
    #expect(requests[1].headers["authorization"] == "Bearer kling-token")
    #expect(requests[1].headers["user-agent"] == "CustomApp/1.0 ai-sdk/klingai/4.0.42")
}

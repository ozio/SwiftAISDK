...
@Test func revAITranscriptionSubmitsMultipartJobAndFetchesTranscript() async throws {
    ...
    #expect(requests[0].headers["user-agent"] == "ai-sdk/revai/3.0.11")
...
@Test func revAITranscriptionAppendsVersionedUserAgentToCustomHeader() async throws {
    ...
    #expect(requests[0].headers["user-agent"] == "CustomApp/1.0 ai-sdk/revai/3.0.11")
    #expect(requests[1].headers["user-agent"] == "CustomApp/1.0 ai-sdk/revai/3.0.11")

...
@Test func cohereEmbeddingAndRerankingPreserveResponseMetadata() async throws {
    ...
    let embedding = try await provider.embeddingModel("embed-v4.0").embed(EmbeddingRequest(values: ["hello"]))
    ...
    let rerank = try await rerankProvider.rerankingModel("rerank-v3.5").rerank(RerankingRequest(query: "q", documents: ["a", "b"]))
...
@Test func voyageMistralBasetenVectorModelsPreserveResponseMetadata() async throws {
    ...
    let voyageEmbedding = try await voyage.embeddingModel("voyage-3").embed(EmbeddingRequest(values: ["hello"]))
    ...
    let voyageRerank = try await voyageRerankProvider.rerankingModel("rerank-2").rerank(RerankingRequest(query: "q", documents: ["a"]))
    ...
    let mistralEmbedding = try await mistral.embeddingModel("mistral-embed").embed(EmbeddingRequest(values: ["hello"]))
    ...
    let basetenEmbedding = try await baseten.embeddingModel("embed").embed(EmbeddingRequest(values: ["hello"]))
...
@Test func gatewayTogetherAndGenericRerankingPreserveResponseMetadata() async throws {
    ...
    let embedding = try await gateway.embeddingModel("text-embedding").embed(EmbeddingRequest(values: ["a"]))
    ...
    let gatewayRerank = try await gatewayRerankProvider.rerankingModel("reranker").rerank(RerankingRequest(query: "q", documents: ["a", "b"]))
    ...
    let togetherRerank = try await together.rerankingModel("Salesforce/Llama-Rank-V1").rerank(RerankingRequest(query: "q", documents: ["a"]))
    ...
    let genericRerank = try await generic.rerank(RerankingRequest(query: "q", documents: ["a"]))
...
@Test func amazonBedrockEmbeddingAndRerankingPreserveResponseMetadata() async throws {
    ...
    let embedding = try await provider.embeddingModel("amazon.titan-embed-text-v2:0").embed(EmbeddingRequest(values: ["hello"]))
    ...
    let rerank = try await rerankProvider.rerankingModel("cohere.rerank-v3-5:0").rerank(RerankingRequest(query: "q", documents: ["a", "b"]))

...
@Test func outputTextRoutesThroughGenerateText() async throws {
    ...
    #expect(model.requests.first?.responseFormat == .text)
}
...
@Test func outputObjectDoesNotParseWhenGenerateTextFinishesWithToolCallsLikeUpstream() async throws {
    ...
    #expect(result.textResult.content == [
        .toolCall(toolCall),
        .toolResult(AIToolResult(toolCallID: "call-1", toolName: "testTool", result: "tool result"))
    ])
    #expect(model.requests.first?.responseFormat == .json(schema: objectFacadeAnswerSchema()))
}
